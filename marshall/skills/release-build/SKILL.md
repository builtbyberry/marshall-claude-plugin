...
- `mcp__plugin_marshall_marshall__heartbeat_claim` — keep the explicit claim's lease
  ...
  **immediately before a long-running operation** (Step 4 build, Step 6's full test /
  ...
  `{ component, agent: "claude-code", machine, worktree }`, determined as
  `/marshall:release-topic`'s **Reporting where you work** says. A `lease_lost` is a
...
- **On a resume or land-only entry only** — a part already in progress — confirm
  ...
  `/marshall:release-topic`'s **reuse rule** checks (its stops come before any
...
Run `/marshall:release-topic`'s procedure: confirm startable, the **reuse rule**
...
a `lease_lost` is a stop → run `/marshall:release-topic`'s **Taking the claim** rule.
...
- **Reuse, don't re-create.** Where the part is worked is decided by
  `/marshall:release-topic`'s **reuse rule**, never restated here; this skill never
