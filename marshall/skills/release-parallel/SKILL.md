...
4. Record the transition: `mcp__plugin_marshall_marshall__dispatch_report { member, status: dispatched,
...
- **Dispatch + report only.** No auto-merge, no auto-wrap, no auto-retry of a
  ...
  `/marshall:release-wrap`), not things the wave does on its own.
