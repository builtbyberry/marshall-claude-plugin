...
- `mcp__plugin_marshall_marshall__release_get` — read the component's `topic_branch`, `hold`, and
  `last_worked`. Always pass `view: "full"`: the summary view you set below omits
  all three.
- `mcp__plugin_marshall_marshall__claim_component` — take the lock (returns the claim id + fence), and
...
- `agent` — always `agent: "claude-code"`.
...
`mcp__plugin_marshall_marshall__release_get { release, view: "full" }` (an older store that returns
...
**Worktree mode** (`--worktree`, as `/marshall:release-parallel` invokes it)
...
1. Identify the component (by tracker ref or title). If unclear, run
   `/marshall:release-next` first and let the user pick. Then decide where the
...
2. Take the claim (**Taking the claim** below). Pass the location fields; include
   ...
   `mcp__plugin_marshall_marshall__claim_component { component: <id>, agent: "claude-code", machine: <machine>, worktree: <worktree> }`
...
3. Only after a successful claim (or on finding you still hold it), mark the work
   started: `mcp__plugin_marshall_marshall__set_component_state { component, state: "in_progress" }` —
   **only when the part is `open`.** A continued part is already `in_progress`
   ...
   `mcp__plugin_marshall_marshall__heartbeat_claim { component: <id>, agent: "claude-code", machine: <machine>, worktree: <worktree> }`.
   Begin work.
...
1. `mcp__plugin_marshall_marshall__my_claims { release }` lists the component → **your actor still holds
   it** — that proves the actor, not this session. Before beating it, read
   `mcp__plugin_marshall_marshall__release_get { release, view: "full" }` and compare the component's
   ...
   - The hold names a **different machine or a different worktree** → another
     ...
     (`mcp__plugin_marshall_marshall__revoke_claim`) and you start again.
...
2. Otherwise read `mcp__plugin_marshall_marshall__release_get { release, view: "full" }` and check the
   component:
...
3. The claim returns `claim_conflict` → someone took it in between. Read
   `mcp__plugin_marshall_marshall__release_get { release, view: "full" }` for `hold.machine` and
